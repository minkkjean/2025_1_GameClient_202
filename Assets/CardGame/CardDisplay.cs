...
public class CardDisplay : MonoBehaviour
{
    ...
    private void OnMouseUp()
    {
        ...
        else if (Physics.Raycast(ray, out hit, Mathf.Infinity, enemyLaYer))
        {
            if (playerStats != null)
            {
                ...
                case CardData.AdditionalEffectType.DiscardCard: //카드 버리기 구현 (랜덤 버리기)
                    for (int i = 0; i < effect.effectAmount; i++)
                    {
                        if (cardManager != null && cardManager.handCards.Count > 0)
                        {
                            ...
                            Debug.Log($"랜덤 카드 버리기 : 선택된 인덱스 {randomIndex}.현대 손패 크기 : {cardManager.handCards.Count}");
                            ...
                            if (cardIndexCopy < cardManager.handCards.Count)
                            {
                                if (randomIndex != cardIndexCopy)
                                {
                                    ...
                                    //만약 버린 카드의 인덱스가 현재 카드의 인덱스보다 작다면 현대 카드의 인덱스를 1 감소 시켜야 함
                                    if (randomIndex < cardIndexCopy)
                                    {
                                        cardIndexCopy--;
                                    }
                            ...
                            {
                                //cardIndexCopy 가 더 이상 유효가지 않은 경우 , 아무 카드나 버림
                                cardManager.DiscardCard(randomIndex);
                            }
                        }
                    }
