...
{
    [SerializeField]
    private string jsonFileNmae = "items";               //Resources 폴더에서 가져올 JSON 파일 이름
    ...
    void LoadItemData()
    {
        TextAsset jsonFile = Resources.Load<TextAsset>(jsonFileNmae); //TextAsset 형태로 Json 파일을 로딩한다.
        ...
        {
            Debug.LogError($"JSON 파일을찾을 수 없습니다. : {jsonFileName}");
        }
